...
def generate_keys(name: str = "key", mode: str = "highest", skip_check: bool = False, debug: bool = False,
    ...
    if debug:
        logger.debug("Starting key generation in %s mode", mode)
    ...
    if check_time:
        ...
        logger.debug(f"Key generation took {elapsed:.4f} seconds")

    if skip_check:
        logger.debug("Skipping security check")
    else:
        ...
        if security_check(N1):
            ...
            if check_time:
                elapsed = time.time() - step_start
                logger.debug(f"Security check took {elapsed:.4f} seconds")
        else:
            logger.warning("Security check failed!")

        if attack_simulation(N1):
            logger.debug("Security 2 check passed")
            if check_time:
                elapsed = time.time() - step_start
                logger.debug(f"Security check 2 took {elapsed:.4f} seconds")
        else:
...
def encrypt(name: str, message: str, check_time: bool = True, debug: bool = False) -> str:
    """
    ...
    """
    logger.debug("Encrypting message with key: %s", name)
    start_time = time.time()

    E = NTRUencrypt(logger=logger, debug=debug)
...
def decrypt(name: str, cipher: str, check_time: bool = True, debug: bool = False) -> str:
    """
    ...
    """
    logger.debug("Decrypting message with key: %s", name)
    start_time = time.time()

    is_debug_mode = (logger.level <= logging.DEBUG)
    D = NTRUdecrypt(logger=logger, debug=debug)
